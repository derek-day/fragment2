...
const BattleSystem = ({ userStats, playerName, page, userId, pageId }) => {
  ...
  useEffect(() => {
    ...
    const initDice = () => {
        // Safety check: ensure container has real width
        const width = containerRef.current.clientWidth;
        const height = containerRef.current.clientHeight;
        
        if (width < 50 || height < 50) return; // Too small, wait for next resize event

        if (diceBoxRef.current) return; // Already initialized
        ...
        const config = {
            ...
            gravity: 10,
            mass: 5,        // Heavy dice
            friction: 0.8,  // High friction stops sliding
            ...
            w: width, // Explicitly set physics world width
            h: height 
        };
    ...
    // Use ResizeObserver to detect when the container actually gets its size
    observer = new ResizeObserver((entries) => {
        for (let entry of entries) {
            if (entry.contentRect.width > 100) {
                // If not initialized, init now that we have size
                if (!diceBoxRef.current) {
                    initDice();
                } else {
                    // Optional: If already init, you could trigger a resize update here if library supports it
                    // But usually just initializing once with correct size is enough for this context
                }
            }
        }
    });
...
//   // Ref to prevent double-execution of NPC turns
//   const turnInProgress = useRef(false);
//   const executedTurns = useRef(new Set()); // Track which turns have been executed
...
//       const nextIndex = (currentTurnIndex + 1) % prevOrder.length;
//       setCurrentTurnIndex(nextIndex);
//       executedTurns.current.clear(); // Clear executed turns when advancing
//       return prevOrder;
//     });
//   };
...
//     // Wait for container to be fully rendered and sized (after any transitions)
//     const initializeDice = () => {
//       const isMobile = window.innerWidth < 768;
      
//       // Get the actual container dimensions for responsive dice rolling
...
//         shadowOpacity: 0.8,
//         throwForce: isMobile ? 0.05 : 0.08,
//         // Use full container dimensions for dice rolling area
//         w: containerWidth,
//         h: containerHeight,
//       };
...
//     // CRITICAL FIX: Check if this specific turn has already been executed
//     const turnKey = `${currentTurn.id}_${currentTurnIndex}`;
//     if (turnInProgress.current || executedTurns.current.has(turnKey)) {
//       return;
//     }
...
//     turnInProgress.current = true;
//     executedTurns.current.add(turnKey); // Mark this turn as executed
...
//       if (currentTurn.type === 'ally') {
//         // Capture current allies state to avoid re-running when state updates
//         setAllies(currentAllies => {
//           const ally = currentAllies.find(a => a.id === currentTurn.id);
//           if (ally && ally.currentHP > 0) {
//             setEnemies(currentEnemies => {
...
//       } 
//       else if (currentTurn.type === 'enemy') {
//         // Capture current enemies state to avoid re-running when state updates
//         setEnemies(currentEnemies => {
//           const enemy = currentEnemies.find(e => e.id === currentTurn.id);
//           if (enemy && enemy.currentHP > 0) {
//             performEnemyTurn(enemy);
...
//     // CRITICAL FIX: Check if this specific turn has already been executed
//     const turnKey = `${currentTurn.id}_${currentTurnIndex}`;
//     if (turnInProgress.current || executedTurns.current.has(turnKey)) return;
