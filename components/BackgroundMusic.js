...
export default function BackgroundMusic({ 
  ...
  // --- Helper: Safe Play ---
  // Wraps play in a promise handler to avoid browser console errors
  const safePlay = () => {
  ...
  // 1. Effect: Handle Track Switching
  useEffect(() => {
    if (!audioRef.current) return;
    if (isPlaying) {
      audioRef.current.pause();
  ...
  // 2. Effect: Initial Mount & User Interaction Listener
  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.volume = volume;

      if (autoPlay) {
        // Attempt 1: Try to play immediately (Works if browser allows it)
        const playPromise = audioRef.current.play();
        if (playPromise !== undefined) {
          playPromise
            .then(() => setIsPlaying(true))
            .catch(() => {
              // Attempt 2: If blocked, wait for first interaction
  ...
  return (
    <motion.div 
      ...
      className="fixed bottom-4 left-4 z-50 flex flex-col items-end pointer-events-none display"
    >
      <audio ref={audioRef} src={currentTrack.src} preload="auto" />

      <motion.div 
        layout
        className="pointer-events-auto backdrop-blur-md border border-gray-700 shadow-2xl overflow-hidden"
        animate={{ width: isMinimized ? "auto" : "300px" }}
        transition={{ type: "spring", stiffness: 300, damping: 30 }}
      >
        {/* Header / Minimized View */}
        <div className="flex items-center justify-between p-3 border-b border-gray-700/50">
          <div className="flex items-center gap-3 overflow-hidden">
            <motion.div className={isPlaying ? "text-blue-400" : "text-gray-500"}>
              <Music size={18} />
            </motion.div>
          ...
          {!isMinimized && (
            ...
            >
              <div className="flex justify-center items-center gap-6">
                <button onClick={() => changeTrack(-1)} className="text-gray-400 hover:text-white transition-colors">
                  <SkipBack size={24} />
                </button>
                <button 
                  onClick={togglePlay}
              ...
              <div className="flex items-center gap-3 pt-2">
                <button onClick={() => setVolume(volume === 0 ? 0.3 : 0)} className="text-gray-400 hover:text-white">
