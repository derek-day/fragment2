...
// import { Plus, Minus, Sparkles, Swords, Wand2, Users, Smile } from "lucide-react";

// export default function PageStats({ onStatsChange }) {
//   // 1. Setup the 4 specific stats starting at base 10
//   const [stats, setStats] = useState({
//     Fellowship: 10,
//     Athletics: 10,
//     Thought: 10,
...
//   // 2. Set total available points to 8
//   const MAX_POINTS = 6;
//   const BASE_STAT_SUM = 40; // 4 stats * 10 base value
...
//     if (pointsRemaining - delta < 0) return;
//     // Prevent going below 1 (though base is usually 10, this is a safety guard)
//     if (stats[stat] + delta < 1) return;
...
//     // 3. Calculate remaining points based on the new total vs the base total
//     // Current Sum - Base Sum (40) = Points Spent
//     const currentSum = Object.values(newStats).reduce((a, b) => a + b, 0);
//     setPointsRemaining(MAX_POINTS - (currentSum - BASE_STAT_SUM));
//   };
...
//   const getClassPrediction = () => {
//     const { Fellowship, Athletics, Thought, Essence } = stats;
//     // Thresholds might need adjusting depending on game balance, 
//     // but 14 is achievable (10 base + 4 points)
//     if (Essence >= 14) return { name: "Mage", icon: Wand2, color: "text-purple-400" };
//     if (Athletics >= 14) return { name: "Warrior", icon: Swords, color: "text-orange-400" };
//     if (Thought >= 14) return { name: "Summoner", icon: Users, color: "text-blue-400" };
//     // You might want to add a Fellowship class check here, e.g., Bard/Diplomat
//     // if (Fellowship >= 14) return { name: "Diplomat", icon: Sparkles, color: "text-yellow-400" };
//     if (Fellowship >= 14) return { name: "Undecided / Mixed", icon: Smile, color: "text-green-400", description: "Lowers cost of items" };
//     return { name: "Undecided / Mixed", icon: Sparkles, color: "text-gray-400" };
//   };
...
//                     disabled={value <= 10} /* Preventing going below base 10, change to 1 if you want to allow dumping stats */
//                   >
//                     <Minus size={16} />
//                   </motion.button>
